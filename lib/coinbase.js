...
// Get the smallest allowed increment for order sizes on this product.
// Coinbase rejects orders with more decimal places than this (e.g. ADA
// might only allow whole numbers while BTC allows 8 decimal places) —
// order amounts must be rounded to a multiple of this before submitting.
async function getBaseIncrement(productId) {
  const data = await cbRequest("GET", `/api/v3/brokerage/products/${productId}`);
  return parseFloat(data.base_increment);
}

// Round a coin amount DOWN to the nearest valid increment for this product,
// so Coinbase doesn't reject the order for having too many decimal places.
...
module.exports = { getSpotPrice, listAccounts, marketSell, marketBuy, getCandles, getBaseIncrement, roundToIncrement };
